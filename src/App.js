...
console.log(actualData);
  ...
  const handleSearch=debounce((value) => {
    console.log(value);
    if(value.length>0){
      let filteredData=data.filter(e=>e.role===value.trim()||e.name.toLowerCase().includes(value.toLowerCase())||e.email.toLowerCase().includes(value.toLowerCase()));
  ...
  return (
    <div>
      ...
      <Button variant="contained" color="error" sx={ { borderRadius: 28,textTransform: 'none',marginTop:1 } } onClick={multipleDEleteHandler}>
                  Delete Selected
            </Button>
      
      <div className='pagination'>
      <Pagination count={count} page={page} color="primary"  onChange={handlePageChange} showLastButton showFirstButton />
      </div>
